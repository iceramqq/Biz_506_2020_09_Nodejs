...
import petDataVO from "../models/pet.js";
import { DATA_GO_SEVICE_KEY, PET_URL } from "../modules/Data.go.kr.js";

const execStation = () => {
  const goDataServiceKey = encodeURIComponent(DATA_GO_SEVICE_KEY);
  const queryString =
    PET_URL + "?" + encodeURIComponent("ServiceKey") + "=" + goDataServiceKey;
  const reqOption = {
    url: queryString,
    method: "GET",
  };
  ...
  request(reqOption, async (err, response, body) => {
    ...
    console.log("데이터 개수 : ", station_list.length);
